"""
widgets.py — OTERNOS PLAYER
All custom tkinter canvas widgets: WaveVisualizer, OternosLogo, MikuLogo,
MikuCornerDeco, MikuTicker, CornerBrackets, DataTicker, MosaicArt,
TrackRecommender, NormMeter.
"""
...
if getattr(sys, "frozen", False):
    from oternos.utils import C, HW_ACCEL
else:
    from .utils import C, HW_ACCEL
    ...
    def _draw(self):
        ...
        if w > 1 and h > 1:
            ...
            for i in range(self.bars):
                edge = abs(i - self.bars / 2) / (self.bars / 2)
                edge_fade = 1.0 - edge**1.6
                if self.active:
                    amp = (
                        math.sin(self.t * self.speeds[i] * 10 + self.phases[i]) * 0.5
                        + 0.5
                    ) ** 1.5
                    amp = max(0.04, amp)
...
class OternosLogo(tk.Canvas):
    ...
    def __init__(self, parent, size=54, bg=None, **kw):
        bg = bg or C["panel"]
        super().__init__(
            parent, bg=bg, highlightthickness=0, width=size, height=size, **kw
        )
        self.size = size
        self._bg = bg
        self.t = 0
    ...
    def _draw(self):
        ...
        try:
            ...
            for i in range(6):
                ...
                tx = cx + R * math.cos(a)
                ty = cy + R * math.sin(a)
                lx = cx + R * 0.68 * math.cos(a - bw)
                ly = cy + R * 0.68 * math.sin(a - bw)
                rx = cx + R * 0.68 * math.cos(a + bw)
                ry = cy + R * 0.68 * math.sin(a + bw)
            ...
            for i in range(6):
                ...
                self.create_line(
                    cx + R * 0.74 * math.cos(a),
                    cy + R * 0.74 * math.sin(a),
                    cx + R * 0.62 * math.cos(a),
            ...
            rOi = R * 0.28
            self.create_oval(
                cx - rO, cy - rO, cx + rO, cy + rO, outline=hc, fill="", width=2
            )
            self.create_oval(
                cx - rOi, cy - rOi, cx + rOi, cy + rOi, outline=dc, fill="", width=1
            )
            ...
            for i in range(12):
                ...
                self.create_line(
                    ...
                    cx + r2 * math.cos(a),
                    cy + r2 * math.sin(a),
                    fill=(mc if long else dc),
                    width=(2 if long else 1),
                )
...
class MikuLogo(tk.Canvas):
    ...
    def __init__(self, parent, size=54, bg=None, **kw):
        ...
        self._sparkles = [
            (i * math.pi * 2 / 6, 0.44 + (i % 2) * 0.06, 0.012 + i * 0.003, i * 1.1)
            for i in range(6)
        ]
        self._draw()
        ...
        try:
            ...
            if pl:
                ...
                self.create_oval(
                    ...
                    cx + hr + 2,
                    cy + hr + 2,
                    outline=f"#0a{gv + 10:02x}{gv:02x}",
                    fill="",
                    width=3,
                )
            self.create_oval(
                cx - hr, cy - hr, cx + hr, cy + hr, outline="#0d4a5e", fill="", width=1
            )
            ...
            if len(pts_l) >= 4:
                ...
                self.create_line(
                    pts_l, fill="#1e8a84", width=4, smooth=True, capstyle="round"
                )
                self.create_line(
                    pts_l, fill="#39c5bb", width=2, smooth=True, capstyle="round"
                )
            ...
            if len(pts_r) >= 4:
                self.create_line(
                    pts_r, fill="#1e8a84", width=4, smooth=True, capstyle="round"
                )
                self.create_line(
                    pts_r, fill="#4dd8d0", width=2, smooth=True, capstyle="round"
                )
            ...
            head_fill = (
                "#071a28"
                if not pl
                else f"#07{int(0x1A + pulse * 8):02x}{int(0x28 + pulse * 10):02x}"
            )
            self.create_oval(
                cx - hrx,
                hy - hry,
...
class MikuCornerDeco(tk.Canvas):
    ...
    def _draw(self):
        ...
        try:
            ...
            pink = (
                f"#{min(255, pv * 3 + 80):02x}{pv // 2:02x}{min(255, pv * 2 + 60):02x}"
            )
            ...
            def _corner(ox, oy, dx, dy, col):
                # main arm
                self.create_line(
                    ox, oy, ox + dx * L, oy, fill=col, width=2, capstyle="round"
                )
                self.create_line(
                    ox, oy, ox, oy + dy * L, fill=col, width=2, capstyle="round"
                )
            ...
            sym_c = (
                f"#{sym_v:02x}{min(255, sym_v * 4):02x}{min(255, sym_v * 4 - 3):02x}"
            )
            self.create_text(
                S // 2,
                S // 2,
...
class MikuTicker(tk.Canvas):
    ...
    def __init__(self, parent, width=200, height=12, bg=None, **kw):
        bg = bg or "#071520"
        super().__init__(
            parent, bg=bg, highlightthickness=0, width=width, height=height, **kw
        )
        self._bg = bg
        self._width = width
        self._height = height
...
class AngelTicker(tk.Canvas):
    """
    Scrolling Angel ticker:
      • Feathers, hearts, stars, crosses and soft angelic phrases
      • Blush pink / rose / lilac coloring
      • Gentle sparkle pulse on each token
    """
    ...
    _COLORS = ("blush", "rose", "lilac", "white")
    ...
    def _gen(self):
        ...
        result = []
        for _ in range(self._width // 20 + 8):
            tok = random.choice(self._TOKENS)
            mode = random.choice(self._COLORS)
            result.append((tok, mode))
        return result
    ...
    def _draw(self):
        ...
        try:
            ...
            for i, (tok, mode) in enumerate(self._data):
                ...
                if x > self._width:
                    break
                fade = max(0.0, min(1.0, x / max(1, self._width * 0.12)))
                flicker = math.sin(t * 0.07 + i * 0.9) * 0.5 + 0.5
                pulse = 0.55 + flicker * 0.45
                if mode == "blush":
                    col = f"#{int(180 * pulse + 50):02x}{int(50 * pulse):02x}{int(120 * pulse + 30):02x}"
                elif mode == "rose":
                    col = f"#{int(200 * pulse + 30):02x}{int(80 * pulse + 20):02x}{int(130 * pulse + 20):02x}"
                elif mode == "lilac":
                    col = f"#{int(150 * pulse + 30):02x}{int(80 * pulse + 20):02x}{int(130 * pulse + 20):02x}"
                else:  # white → soft ink on light bg
                    v = int(30 + fade * 60)
                    col = f"#{v:02x}{int(v * 0.5):02x}{int(v * 0.75):02x}"
                self.create_text(
                    x + 2,
                    self._height // 2,
                    text=tok,
                    font=("Palatino Linotype", 6) if True else ("Georgia", 6),
                    fill=col,
                    anchor="w",
                )
        except Exception:
            pass
        if getattr(self, "_alive", True):
            self.after(52 if HW_ACCEL else 999, self._draw)
...
class DataTicker(tk.Canvas):
    ...
    def _gen(self):
        ...
        return [f"{random.randint(0, 0xFFFF):04X}" for _ in range(self._cols + 4)]
...
class TrackRecommender:
    ...
    def recommend(self, n=20):
        ...
        played = {
            (e.get("artist", "").lower(), e.get("title", "").lower()) for e in recent
        }
        ...
        if len(result) < n:
            ...
            remaining = [
                ...
                and (
                    ...
                    self.library[i].get("title", "").lower(),
                )
                not in played
            ]
...
class NormMeter:
    ...
    def __init__(self, parent, width=180, height=8):
        self.cv = tk.Canvas(
            parent, bg=C["panel"], width=width, height=height, highlightthickness=0
        )
        self._w = width
        self._h = height
...
# ─────────────────────────────────────────
#  NieR LOGO — YoRHa hexagonal sigil
# ─────────────────────────────────────────
class NierLogo(tk.Canvas):
    """
    Animated YoRHa emblem:
      • Outer hex ring — rotates slowly, pulses with music
      • Inner hex — counter-rotates
      • Cross lines through centre
      • Pulsing core dot
      • All rendered in aged gold/cream tones matching NieR palette
    """
    ...
    def _draw(self):
        ...
        try:
            ...
            pulse = (math.sin(t * 0.07) * 0.5 + 0.5) if pl else 0.2
            speed = 0.018 if pl else 0.005
            ang = t * speed

            # Outer hex ring
            r_out = S * (0.44 + pulse * 0.03)
            gv = int(140 + pulse * 80)
            pts_outer = []
            for k in range(6):
                a = ang + k * math.pi / 3
                pts_outer += [cx + r_out * math.cos(a), cy + r_out * math.sin(a)]
            cv = int(gv * 0.55)
            self.create_polygon(
                pts_outer,
                outline=f"#{gv:02x}{int(gv * 0.90):02x}{cv:02x}",
                fill="",
                width=1,
            )

            # Inner hex — counter-rotates
            r_in = S * (0.28 + pulse * 0.02)
            mv = int(80 + pulse * 60)
            pts_inner = []
            for k in range(6):
                a = -ang * 1.4 + k * math.pi / 3
                pts_inner += [cx + r_in * math.cos(a), cy + r_in * math.sin(a)]
            self.create_polygon(
                pts_inner,
                outline=f"#{mv:02x}{int(mv * 0.90):02x}{int(mv * 0.52):02x}",
                fill="",
                width=1,
            )

            # Cross arms
            arm = S * (0.34 + pulse * 0.02)
            lv = int(100 + pulse * 90)
            lc = f"#{lv:02x}{int(lv * 0.88):02x}{int(lv * 0.50):02x}"
            self.create_line(cx - arm, cy, cx + arm, cy, fill=lc, width=1)
            self.create_line(cx, cy - arm, cx, cy + arm, fill=lc, width=1)

            # Diagonal tick marks at hex vertices (inner ring detail)
            for k in range(6):
                a = ang + k * math.pi / 3
                r1 = r_out * 0.78
                r2 = r_out * 0.64
                dv = int(50 + pulse * 40)
                dc = f"#{dv:02x}{int(dv * 0.88):02x}{int(dv * 0.48):02x}"
                self.create_line(
                    cx + r1 * math.cos(a),
                    cy + r1 * math.sin(a),
                    cx + r2 * math.cos(a),
            ...
            # Core dot
            cr = max(2, S * (0.05 + pulse * 0.03))
            cv2 = int(190 + pulse * 60)
            self.create_oval(
                cx - cr,
                cy - cr,
                cx + cr,
                cy + cr,
                fill=f"#{cv2:02x}{int(cv2 * 0.90):02x}{int(cv2 * 0.55):02x}",
                outline="",
            )
...
# ─────────────────────────────────────────
#  BAROQUE FRAME OVERLAY  (Angel theme)
# ─────────────────────────────────────────


class BaroqueStrip(tk.Canvas):
    """
    A thin animated strip of baroque ornaments along one edge of the window.
    Draws only on a narrow band (THICKNESS px) so nothing covers the UI interior.
    bg matches the theme panel/bg color so it blends with the existing border.
    """

    THICKNESS = 18  # px wide — just enough for border lines + dot chain
    ...
    def __init__(self, parent, edge="top", length=800, bg="#f7edf5", **kw):
        ...
        w = length if horizontal else self.THICKNESS
        h = self.THICKNESS if horizontal else length
        super().__init__(
            parent, width=w, height=h, bg=bg, highlightthickness=0, bd=0, **kw
        )
        self._length = length
        self.t = 0
        self._scroll = 0.0
    ...
    def _c(self, hex6, pulse, lo=0.45, hi=1.0):
        r = int(hex6[1:3], 16)
        g = int(hex6[3:5], 16)
        b = int(hex6[5:7], 16)
        f = lo + (hi - lo) * pulse
        return f"#{min(int(r * f), 255):02x}{min(int(g * f), 255):02x}{min(int(b * f), 255):02x}"
    ...
    def _draw(self):
        ...
        try:
            ...
            L = self._length
            T = self.THICKNESS
            pulse = math.sin(t * 0.045) * 0.5 + 0.5
            pulse2 = math.sin(t * 0.03 + 1.0) * 0.5 + 0.5
            B = self.BLUSH
            R = self.ROSE
            G = self.GOLD
            D = self.DIM
            ...
            # Which side the border line sits on
            # For top strip: line at bottom edge (y = T-2 and T-1)
            # For bottom strip: line at top edge (y = 1 and 2)
            # For left strip: line at right edge; for right strip: line at left edge
            if edge == "top":
                ly1, ly2 = T - 3, T - 1
            elif edge == "bottom":
                ly1, ly2 = 1, 3
            ...
            if horiz:
                # ── Thick border line ──
                self.create_line(
                    0, ly2, L, ly2, fill=self._c(G, pulse, 0.4, 0.75), width=2
                )
                # ── Hairline ──
                self.create_line(
                    0, ly1, L, ly1, fill=self._c(D, pulse, 0.25, 0.55), width=1
                )

                # ── Scrolling dot chain ──
                self._scroll += 0.35
                step = 22
                off = -(self._scroll % step)
                x = off
                toggle = 0
                while x < L:
                    dr = 2.2 if toggle % 3 == 0 else 1.2
                    col = self._c(
                        B if toggle % 2 == 0 else R,
                        pulse * (0.4 + 0.3 * math.sin(t * 0.06 + x * 0.04)),
                        0.3,
                        0.8,
                    )
                    dot_y = ly2 - 6
                    self.create_oval(
                        x - dr, dot_y - dr, x + dr, dot_y + dr, fill=col, outline=""
                    )
                    x += step
                    toggle += 1

                # ── Corner roses (left and right ends) ──
                for rx in [T // 2, L - T // 2]:
                    rose_r = 5 + pulse * 1.5
                    dot_y = ly2 - 8
                    for p in range(5):
                        a = t * 0.009 + p * (math.pi * 2 / 5)
                        px = rx + math.cos(a) * rose_r * 0.55
                        py = dot_y + math.sin(a) * rose_r * 0.28
                        pr = rose_r * (0.32 + pulse * 0.08)
                        self.create_oval(
                            px - pr,
                            py - pr,
                            px + pr,
                            py + pr,
                            outline=self._c(B, pulse, 0.5, 0.9),
                            fill=self._bg,
                            width=1,
                        )
                    cr = rose_r * 0.14
                ...
                lz = 28
                dot_y = ly2 - 7
                pts = [mx - lz, dot_y, mx, dot_y - 6, mx + lz, dot_y, mx, dot_y + 6]
                self.create_polygon(
                    pts, outline=self._c(G, pulse2, 0.35, 0.7), fill="", width=1
                )

            else:  # vertical strip (left / right)
                # ── Thick border line ──
                self.create_line(
                    lx2, 0, lx2, L, fill=self._c(G, pulse, 0.4, 0.75), width=2
                )
                # ── Hairline ──
                self.create_line(
                    lx1, 0, lx1, L, fill=self._c(D, pulse, 0.25, 0.55), width=1
                )
                ...
                while y < L:
                    ...
                    col = self._c(
                        B if toggle % 2 == 0 else R,
                        pulse * (0.4 + 0.3 * math.sin(t * 0.06 + y * 0.04)),
                        0.3,
                        0.8,
                    )
                    self.create_oval(
                        dot_x - dr, y - dr, dot_x + dr, y + dr, fill=col, outline=""
                    )
                    y += step
                    toggle += 1
                ...
                lz = 22
                dot_x2 = lx2 - 7
                pts = [dot_x2, my - lz, dot_x2 - 6, my, dot_x2, my + lz, dot_x2 + 6, my]
                self.create_polygon(
                    pts, outline=self._c(G, pulse2, 0.35, 0.7), fill="", width=1
                )
        ...
        self.t += 1
        if self._alive:
            self._job = self.after(40 if HW_ACCEL else 200, self._draw)
...
class BaroqueFrame:
    """
    Places four thin BaroqueStrip canvases along all four edges of the window.
    Each strip is only THICKNESS px wide/tall so nothing covers the UI interior.
    Call .destroy() to remove all strips cleanly.
    """

    def __init__(self, root, bg="#f7edf5"):
        self._root = root
        self._strips = []

        configs = [
            # edge,     relx, rely, anchor, relw,  relh,  x, y
            ("top", 0, 0, "nw", 1.0, None, 0, 0),
            ("bottom", 0, 1.0, "sw", 1.0, None, 0, 0),
            ("left", 0, 0, "nw", None, 1.0, 0, 0),
            ("right", 1.0, 0, "ne", None, 1.0, 0, 0),
        ]
        for edge, relx, rely, anchor, relw, relh, x, y in configs:
            s = BaroqueStrip(root, edge=edge, bg=bg, length=800)
            place_kw = dict(relx=relx, rely=rely, anchor=anchor, x=x, y=y)
            if relw:
                place_kw["relwidth"] = relw
            if relh:
                place_kw["relheight"] = relh
            s.place(**place_kw)
            s.lift()
