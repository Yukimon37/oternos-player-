...
animation.py
─────────────────────────────────────────────────────────────────────────────
OTERNOS PLAYER  —  Widget animation helpers.
  • ColorAnim  — smooth fg/bg colour transitions
  • FadeOverlay — view-switch flash stub
─────────────────────────────────────────────────────────────────────────────
"""

from .constants import lerp_color, ease_out
...
class ColorAnim:
    ...
    def run(cls, root, widget, attr, from_col, to_col,
            duration_ms=120, fps=60, ease_fn=ease_out, on_done=None):
        wid = id(widget)
        if wid in cls._jobs:
...
class FadeOverlay:
    """Stub — calls the midpoint callback immediately. No canvas placement."""
