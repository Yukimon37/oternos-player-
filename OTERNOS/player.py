...
player.py
─────────────────────────────────────────────────────────────────────────────
OTERNOS PLAYER  —  VoidPlayer main application class.
All UI construction, view routing, playback control, and feature logic lives
here. Import and instantiate via __main__.py.
─────────────────────────────────────────────────────────────────────────────
"""
...
from .constants import (
    C, HW_ACCEL, FM, FMS, FML, FMX,
    DATA_FILE, YT_CACHE,
    ease_out, lerp_color,
    # legacy underscore aliases still used inside the class body
    _ease_out, _lerp_color,
    ICON_B64 as _ICON_B64,
)
from .animation  import ColorAnim, FadeOverlay
from .audio      import AudioEngine, EQProcessor, MUTAGEN_AVAILABLE, SCIPY_AVAILABLE
from .services   import GlobalHotkeys, LastFmScrobbler, DiscordRPC, FolderWatcher
from .streaming  import SpotifyAuth, SpotifyAPI, SoundCloudAPI, YouTubeAPI
from .widgets    import (
    WaveVisualizer, OternosLogo, MikuLogo, MikuCornerDeco,
    MikuTicker, CornerBrackets, DataTicker, MosaicArt,
...
class VoidPlayer:
    def __init__(self, root):
        ...
        self._sc_tracks     = []
        self._sc_view       = "search"   # search | likes
    ...
    def _open_settings(self):
        ...
        tk.Label(hdr, text="[*] OTERNOS SETTINGS", font=FML,
                 fg=C["white"], bg=C["panel"]).pack(side="left", padx=18, pady=10)
        cl = tk.Label(hdr, text="[X]", font=("Courier New", 13), fg=C["white3"],
        ...
        # ── footer ──────────────────────────────────────────────────────
        foot = tk.Frame(win, bg=C["panel"], height=44)
        foot.pack(fill="x", side="bottom")
        foot.pack_propagate(False)
        tk.Frame(foot, bg=C["border"], height=1).pack(fill="x")
        save_btn = tk.Label(foot, text="[ SAVE SETTINGS ]", font=FM,
                            fg=C["white3"], bg=C["panel"], cursor="hand2")
        save_btn.pack(side="right", padx=18, pady=10)
        ...
        reset_btn = tk.Label(foot, text="[ RESET ]", font=FM,
                             fg=C["white3"], bg=C["panel"], cursor="hand2")
        reset_btn.pack(side="left", padx=18, pady=10)
        reset_btn.bind("<Button-1>", lambda e: self._open_settings())  # reopen fresh
        ...
        def row(label, widget_fn, key, hint=""):
            ...
            rf.pack(fill="x", padx=24, pady=5)
            lf = tk.Frame(rf, bg=C["bg"])
            lf.pack(side="left", padx=(0, 16))
            tk.Label(lf, text=label, font=("Courier New", 10), fg=C["white2"],
                     bg=C["bg"], anchor="w", width=22, justify="left").pack(anchor="w")
            if hint:
                tk.Label(lf, text=hint, font=("Courier New", 8),
    ...
    def _build_soundcloud_view(self):
        ...
        nav.pack(fill="x", padx=20, pady=(8, 0))
        self._sc_nav_btns = {}
        for label, key in [("SEARCH", "search"), ("LIKES", "likes")]:
            b = tk.Label(nav, text=label, font=FMS, fg=C["white3"],
                         bg=C["bg"], cursor="hand2", padx=10)
            b.pack(side="left")
    ...
    def _sc_show_tracks(self, tracks, label=""):
        ...
        for i, t in enumerate(tracks):
            ...
            self.sc_list.insert("end",
                f"  {i+1:>3}.  {title:<38}  {artist:<26}  {dur_str}")
    ...
    def _build_claude_view(self):
        ...
        tk.Label(top, text="system diagnostic interface", font=FMS, fg=C["white3"], bg=C["bg"]).pack(side="left", padx=12)
        tk.Frame(self.claude_frame, bg=C["border"], height=1).pack(fill="x", pady=(8, 0))

        kf = tk.Frame(self.claude_frame, bg=C["bg"])
        kf.pack(fill="x", padx=20, pady=(8, 0))
    ...
    def _history_add(self, track):
        ...
        if self._history and self._history[-1].get("title") == entry["title"]:
            return
