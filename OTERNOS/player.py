...
if getattr(sys, "frozen", False):
    ...
    from oternos.theme.presets import THEME_PRESETS
    ...
    from .theme.presets import THEME_PRESETS
...
class VoidPlayer:
    ...
    # ── Forwarding properties → controllers ──────────────────────────────────
    # These keep every existing call-site (self.library, self.queue, etc.)
    # working unchanged while the underlying data lives in the controllers.

    @property
    def library(self):
        return self.lib_ctrl.library
    @library.setter
    def library(self, v):
        self.lib_ctrl.library = v

    @property
    def playlists(self):
        return self.lib_ctrl.playlists
    @playlists.setter
    def playlists(self, v):
        self.lib_ctrl.playlists = v

    @property
    def settings(self):
        return self.lib_ctrl.settings
    @settings.setter
    def settings(self, v):
        self.lib_ctrl.settings = v

    @property
    def _history(self):
        return self.lib_ctrl.history
    @_history.setter
    def _history(self, v):
        self.lib_ctrl.history = v

    @property
    def _history_max(self):
        return 200

    @property
    def _bookmarks(self):
        return self.lib_ctrl.bookmarks
    @_bookmarks.setter
    def _bookmarks(self, v):
        self.lib_ctrl.bookmarks = v

    @property
    def _album_meta(self):
        return self.lib_ctrl.album_meta
    @_album_meta.setter
    def _album_meta(self, v):
        self.lib_ctrl.album_meta = v

    @property
    def _sc_favorites(self):
        return self.lib_ctrl.sc_favorites
    @_sc_favorites.setter
    def _sc_favorites(self, v):
        self.lib_ctrl.sc_favorites = v

    @property
    def shuffle(self):
        return self.lib_ctrl.shuffle
    @shuffle.setter
    def shuffle(self, v):
        self.lib_ctrl.shuffle = v

    @property
    def repeat_mode(self):
        return self.lib_ctrl.repeat_mode
    @repeat_mode.setter
    def repeat_mode(self, v):
        self.lib_ctrl.repeat_mode = v

    @property
    def queue(self):
        return self.queue_ctrl.auto
    @queue.setter
    def queue(self, v):
        self.queue_ctrl.auto = v

    @property
    def queue_pos(self):
        return self.queue_ctrl.pos
    @queue_pos.setter
    def queue_pos(self, v):
        self.queue_ctrl.pos = v

    @property
    def _manual_queue(self):
        return self.queue_ctrl.manual
    @_manual_queue.setter
    def _manual_queue(self, v):
        self.queue_ctrl.manual = v

    # ── Source forwarding properties ──────────────────────────────────────────
    @property
    def _active_source(self) -> str:
        return self.source_ctrl.active_str

    @_active_source.setter
    def _active_source(self, v: str):
        # _active.__class__ is Source — use it to parse string without re-importing
        self.source_ctrl.switch_to(self.source_ctrl.active.__class__.from_str(v))

    @property
    def _sp_mode(self) -> bool:
        return self.source_ctrl.is_spotify

    @_sp_mode.setter
    def _sp_mode(self, v: bool):
        # Setting True → switch to Spotify; setting False → use stop_all() instead
        if v:
            self.source_ctrl.switch_to(self.source_ctrl.active.__class__("spotify"))

    # ── Playback forwarding properties ───────────────────────────────────────
    @property
    def _playback_speed(self):
        return self.playback_ctrl._playback_speed
    @_playback_speed.setter
    def _playback_speed(self, v):
        self.playback_ctrl._playback_speed = v

    @property
    def _speed_processing(self):
        return self.playback_ctrl._speed_processing
    @_speed_processing.setter
    def _speed_processing(self, v):
        self.playback_ctrl._speed_processing = v

    @property
    def _xfade_job(self):
        return self.playback_ctrl._xfade_job
    @_xfade_job.setter
    def _xfade_job(self, v):
        self.playback_ctrl._xfade_job = v

    @property
    def _xfade_fading_in(self):
        return self.playback_ctrl._xfade_fading_in
    @_xfade_fading_in.setter
    def _xfade_fading_in(self, v):
        self.playback_ctrl._xfade_fading_in = v

    @property
    def _xfade_loading(self):
        return self.playback_ctrl._xfade_loading
    @_xfade_loading.setter
    def _xfade_loading(self, v):
        self.playback_ctrl._xfade_loading = v

    @property
    def _ab_a(self):
        return self.playback_ctrl._ab_a
    @_ab_a.setter
    def _ab_a(self, v):
        self.playback_ctrl._ab_a = v

    @property
    def _ab_b(self):
        return self.playback_ctrl._ab_b
    @_ab_b.setter
    def _ab_b(self, v):
        self.playback_ctrl._ab_b = v

    @property
    def _ab_active(self):
        return self.playback_ctrl._ab_active
    @_ab_active.setter
    def _ab_active(self, v):
        self.playback_ctrl._ab_active = v
    ...
    def __init__(self, root):
        ...
        # ── Phase 1 controllers ────────────────────────────────────────────
        if getattr(sys, "frozen", False):
            from oternos.controllers.library import LibraryController
            from oternos.controllers.queue import QueueController
            from oternos.controllers.source import SourceController, Source
            from oternos.controllers.playback import PlaybackController
        else:
            from .controllers.library import LibraryController
            from .controllers.queue import QueueController
            from .controllers.source import SourceController, Source
            from .controllers.playback import PlaybackController
        self.lib_ctrl = LibraryController(self)
        self.queue_ctrl = QueueController(self)
        ...
        # _sp_mode and _active_source are now forwarding properties → source_ctrl
        self._sp_pos_ms = 0
        self._sp_dur_ms = 0
        ...
        # Theme editor
        self._theme_presets = THEME_PRESETS
        ...
        self.root.bind("<space>", _guard(self._toggle_play))
        self.root.bind("<KeyPress-n>", _guard(self._next))
        self.root.bind("<KeyPress-p>", _guard(self._prev))
        self.root.bind("<Right>", _guard(lambda: self._seek_relative(5)))
        self.root.bind("<Left>", _guard(lambda: self._seek_relative(-5)))
        self.root.bind("<Shift-Right>", _guard(lambda: self._seek_relative(30)))
        self.root.bind("<Shift-Left>", _guard(lambda: self._seek_relative(-30)))
        self.root.bind(
            "<Up>",
            _guard(
                lambda: (
    ...
    def _on_watcher_new_file(self, path: str) -> None:
        self.lib_ctrl.import_paths([path])
    ...
    def _refresh_pl_sidebar(self):
        ...
        for name in self.playlists:
            self._sbtn(
                self.pl_sidebar,
                name,
                lambda n=name: self._switch_view("playlist", n),
            )

    def _open_sleep_timer(self):
        if hasattr(self, "_sleep_win") and self._sleep_win and self._sleep_win.winfo_exists():
            self._sleep_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // SLEEP TIMER")
        win.configure(bg=C["bg"]); win.geometry("320x200"); win.resizable(False, False)
        self._sleep_win = win
        tk.Label(win, text="SLEEP TIMER", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=20)
        row = tk.Frame(win, bg=C["bg"]); row.pack()
        tk.Label(row, text="Minutes:", font=FM, fg=C["white2"], bg=C["bg"]).pack(side="left")
        var = tk.StringVar(value="30")
        tk.Entry(row, textvariable=var, width=6, font=FM, bg=C["panel"], fg=C["white"],
                 insertbackground=C["white"], relief="flat", bd=4).pack(side="left", padx=8)
        def _set():
            try:
                mins = int(var.get())
                if hasattr(self, "_sleep_job") and self._sleep_job:
                    self.root.after_cancel(self._sleep_job)
                self._sleep_job = self.root.after(mins * 60000, self._on_close)
                win.destroy()
            except ValueError:
                pass
        tk.Button(win, text="SET", font=FM, fg=C["white"], bg=C["border"],
                  relief="flat", bd=0, command=_set, cursor="hand2").pack(pady=14)
        def _cancel():
            if hasattr(self, "_sleep_job") and self._sleep_job:
                self.root.after_cancel(self._sleep_job); self._sleep_job = None
            win.destroy()
        tk.Button(win, text="CANCEL TIMER", font=FMS, fg=C["white2"], bg=C["bg"],
                  relief="flat", bd=0, command=_cancel, cursor="hand2").pack()

    def _toggle_mini_player(self):
        pass  # mini player not yet implemented in this build

    def _open_watch_dirs(self):
        if hasattr(self, "_watch_win") and self._watch_win and self._watch_win.winfo_exists():
            self._watch_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // WATCH FOLDERS")
        win.configure(bg=C["bg"]); win.geometry("480x360"); win.resizable(False, False)
        self._watch_win = win
        tk.Label(win, text="WATCH FOLDERS", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=14)
        tk.Label(win, text="New audio files added to these folders\nare imported automatically.",
                 font=FMS, fg=C["white2"], bg=C["bg"]).pack()
        lb = tk.Listbox(win, font=FM, bg=C["panel"], fg=C["white"], selectbackground=C["select"],
                        relief="flat", bd=0, height=8)
        lb.pack(fill="x", padx=20, pady=10)
        dirs = self.settings.get("watch_dirs", [])
        for d in dirs: lb.insert("end", d)
        def _add():
            from tkinter import filedialog
            d = filedialog.askdirectory(title="Select Watch Folder")
            if d and d not in dirs:
                dirs.append(d); lb.insert("end", d)
                self.settings["watch_dirs"] = dirs; self._save()
                self._watcher.watch(d)
        def _remove():
            sel = lb.curselection()
            if sel:
                d = lb.get(sel[0]); dirs.remove(d); lb.delete(sel[0])
                self.settings["watch_dirs"] = dirs; self._save()
        bf = tk.Frame(win, bg=C["bg"]); bf.pack()
        tk.Button(bf, text="ADD FOLDER", font=FM, fg=C["white"], bg=C["border"],
                  relief="flat", bd=0, command=_add, cursor="hand2").pack(side="left", padx=6)
        tk.Button(bf, text="REMOVE", font=FM, fg=C["white2"], bg=C["bg"],
                  relief="flat", bd=0, command=_remove, cursor="hand2").pack(side="left", padx=6)

    def _open_smart_playlist(self):
        if hasattr(self, "_smart_win") and self._smart_win and self._smart_win.winfo_exists():
            self._smart_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // SMART PLAYLIST")
        win.configure(bg=C["bg"]); win.geometry("440x300"); win.resizable(False, False)
        self._smart_win = win
        tk.Label(win, text="SMART PLAYLIST", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=20)
        tk.Label(win, text="Filter by genre, BPM, or mood to auto-build a playlist.",
                 font=FMS, fg=C["white2"], bg=C["bg"]).pack()
        row = tk.Frame(win, bg=C["bg"]); row.pack(pady=16)
        tk.Label(row, text="Name:", font=FM, fg=C["white2"], bg=C["bg"]).grid(row=0, column=0, sticky="w", pady=4)
        name_var = tk.StringVar()
        tk.Entry(row, textvariable=name_var, width=22, font=FM, bg=C["panel"], fg=C["white"],
                 insertbackground=C["white"], relief="flat", bd=4).grid(row=0, column=1, padx=8)
        def _create():
            n = name_var.get().strip()
            if n and n not in self.playlists:
                self.playlists[n] = list(range(len(self.library)))
                self._save(); self._refresh_pl_sidebar(); win.destroy()
        tk.Button(win, text="CREATE FROM ALL TRACKS", font=FM, fg=C["white"], bg=C["border"],
                  relief="flat", bd=0, command=_create, cursor="hand2").pack(pady=10)

    def _open_duplicate_detector(self):
        if hasattr(self, "_dupe_win") and self._dupe_win and self._dupe_win.winfo_exists():
            self._dupe_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // DUPLICATE DETECTOR")
        win.configure(bg=C["bg"]); win.geometry("540x400"); win.resizable(False, False)
        self._dupe_win = win
        tk.Label(win, text="DUPLICATE DETECTOR", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=14)
        seen = {}; dupes = []
        for i, t in enumerate(self.library):
            key = (t.get("title", "").lower().strip(), t.get("artist", "").lower().strip())
            if key in seen: dupes.append((seen[key], i, t))
            else: seen[key] = i
        if not dupes:
            tk.Label(win, text="No duplicates found.", font=FM, fg=C["white2"], bg=C["bg"]).pack(pady=30)
            return
        tk.Label(win, text=f"{len(dupes)} duplicate(s) found:", font=FM, fg=C["white2"], bg=C["bg"]).pack()
        lb = tk.Listbox(win, font=FMS, bg=C["panel"], fg=C["white"], selectbackground=C["select"],
                        relief="flat", bd=0, height=12)
        lb.pack(fill="both", expand=True, padx=20, pady=10)
        for _, i, t in dupes:
            lb.insert("end", f"{t.get('artist','')} — {t.get('title','')}")

    def _export_playlist_m3u(self):
        from tkinter import filedialog
        path = filedialog.asksaveasfilename(defaultextension=".m3u",
            filetypes=[("M3U Playlist", "*.m3u")], title="Export Playlist")
        if not path: return
        idxs = self.playlists.get(self.active_playlist, []) if (
            hasattr(self, "active_playlist") and self.active_playlist) else range(len(self.library))
        tracks = [self.library[i] for i in idxs if i < len(self.library)]
    ...
    def _open_hotkey_editor(self):
        if hasattr(self, "_hk_win") and self._hk_win and self._hk_win.winfo_exists():
            self._hk_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // HOTKEYS")
        win.configure(bg=C["bg"]); win.geometry("400x320"); win.resizable(False, False)
        self._hk_win = win
        tk.Label(win, text="KEYBOARD SHORTCUTS", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=14)
        shortcuts = [
            ("Space", "Play / Pause"), ("Right", "Next Track"), ("Left", "Previous Track"),
            ("Up / Down", "Volume"), ("S", "Shuffle"), ("R", "Repeat"),
            ("Ctrl+F", "Search"), ("Ctrl+,", "Settings"), ("Esc", "Close dialog"),
        ]
        for key, action in shortcuts:
            row = tk.Frame(win, bg=C["bg"]); row.pack(fill="x", padx=30, pady=2)
            tk.Label(row, text=key, font=FM, fg=C["white"], bg=C["border"],
                     width=12, anchor="center").pack(side="left")
            tk.Label(row, text=action, font=FM, fg=C["white2"], bg=C["bg"],
                     anchor="w", padx=12).pack(side="left")

    def _open_recommendations(self):
        if hasattr(self, "_rec_win") and self._rec_win and self._rec_win.winfo_exists():
            self._rec_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // RECOMMENDATIONS")
        win.configure(bg=C["bg"]); win.geometry("460x400"); win.resizable(False, False)
        self._rec_win = win
        tk.Label(win, text="RECOMMENDATIONS", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=14)
        cur = self.library[self.current_idx] if (hasattr(self, "current_idx") and
              self.library and 0 <= getattr(self, "current_idx", -1) < len(self.library)) else None
        if cur:
            tk.Label(win, text=f"Based on: {cur.get('artist','')} — {cur.get('title','')}",
                     font=FMS, fg=C["white2"], bg=C["bg"]).pack()
        artist = cur.get("artist", "").lower() if cur else ""
        matches = [t for t in self.library if t.get("artist","").lower() == artist and t != cur][:10]
        if not matches:
            tk.Label(win, text="No similar tracks found in library.", font=FM,
                     fg=C["white2"], bg=C["bg"]).pack(pady=20)
        else:
            lb = tk.Listbox(win, font=FM, bg=C["panel"], fg=C["white"],
                            selectbackground=C["select"], relief="flat", bd=0, height=12)
            lb.pack(fill="both", expand=True, padx=20, pady=10)
            for t in matches: lb.insert("end", f"{t.get('artist','')} — {t.get('title','')}")
            def _play(e):
                sel = lb.curselection()
                if sel:
                    idx = self.library.index(matches[sel[0]])
                    self._play_item(idx)
            lb.bind("<Double-Button-1>", _play)

    def _open_theme_editor(self):
        if hasattr(self, "_theme_win") and self._theme_win and self._theme_win.winfo_exists():
            self._theme_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // THEMES")
        win.configure(bg=C["bg"]); win.geometry("380x420"); win.resizable(False, False)
        self._theme_win = win
        tk.Label(win, text="THEME PRESETS", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=14)
        if hasattr(self, "_theme_presets") and self._theme_presets:
            for name, palette in self._theme_presets.items():
                btn = tk.Button(win, text=name, font=FM, fg=C["white"], bg=C["panel"],
                                relief="flat", bd=0, cursor="hand2",
                                command=lambda p=palette: self._apply_theme(p))
                btn.pack(fill="x", padx=30, pady=3)
        else:
            tk.Label(win, text="No theme presets available.", font=FM,
                     fg=C["white2"], bg=C["bg"]).pack(pady=20)
    ...
    def _open_bookmarks(self):
        if hasattr(self, "_bm_win") and self._bm_win and self._bm_win.winfo_exists():
            self._bm_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // BOOKMARKS")
        win.configure(bg=C["bg"]); win.geometry("460x360"); win.resizable(False, False)
        self._bm_win = win
        tk.Label(win, text="BOOKMARKS", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=14)
        bm = getattr(self, "bookmarks", {})
        if not bm:
            tk.Label(win, text="No bookmarks saved.", font=FM, fg=C["white2"], bg=C["bg"]).pack(pady=20)
            return
        lb = tk.Listbox(win, font=FM, bg=C["panel"], fg=C["white"],
                        selectbackground=C["select"], relief="flat", bd=0, height=12)
        lb.pack(fill="both", expand=True, padx=20, pady=10)
        entries = []
        for lib_idx, marks in bm.items():
            if lib_idx < len(self.library):
                t = self.library[lib_idx]
                for secs, label in marks:
                    entries.append((lib_idx, secs))
                    lb.insert("end", f"{t.get('title','')} @ {int(secs//60)}:{int(secs%60):02d}  {label}")
        def _jump(e):
            sel = lb.curselection()
            if sel:
                lib_idx, secs = entries[sel[0]]
                self._play_item(lib_idx)
    ...
    def _open_auto_playlist(self):
        if hasattr(self, "_auto_win") and self._auto_win and self._auto_win.winfo_exists():
            self._auto_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // AUTO-PLAYLIST")
        win.configure(bg=C["bg"]); win.geometry("400x260"); win.resizable(False, False)
        self._auto_win = win
        tk.Label(win, text="AUTO-PLAYLIST", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=14)
        tk.Label(win, text="Automatically queue tracks by artist or genre.",
                 font=FMS, fg=C["white2"], bg=C["bg"]).pack()
        row = tk.Frame(win, bg=C["bg"]); row.pack(pady=16)
        tk.Label(row, text="Artist:", font=FM, fg=C["white2"], bg=C["bg"]).grid(row=0, column=0, sticky="w", pady=4)
        artist_var = tk.StringVar()
        tk.Entry(row, textvariable=artist_var, width=22, font=FM, bg=C["panel"], fg=C["white"],
                 insertbackground=C["white"], relief="flat", bd=4).grid(row=0, column=1, padx=8)
        def _go():
            a = artist_var.get().strip().lower()
            idxs = [i for i, t in enumerate(self.library) if t.get("artist","").lower() == a]
            if idxs:
                self.queue = idxs; self.queue_pos = 0
                self._play_item(idxs[0]); win.destroy()
        tk.Button(win, text="QUEUE ALL BY ARTIST", font=FM, fg=C["white"], bg=C["border"],
                  relief="flat", bd=0, command=_go, cursor="hand2").pack(pady=10)

    def _open_ambient_mode(self):
        pass  # ambient mode not yet implemented in this build

    def _open_palette(self):
        if hasattr(self, "_palette_win") and self._palette_win and self._palette_win.winfo_exists():
            self._palette_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // COMMAND PALETTE")
        win.configure(bg=C["bg"]); win.geometry("500x60"); win.resizable(False, False)
        self._palette_win = win
        var = tk.StringVar()
        e = tk.Entry(win, textvariable=var, font=FML, bg=C["panel"], fg=C["white"],
                     insertbackground=C["white"], relief="flat", bd=8)
        e.pack(fill="both", expand=True); e.focus()
        win.bind("<Escape>", lambda _: win.destroy())

    def _open_lastfm_auth(self):
        if hasattr(self, "_lfm_win") and self._lfm_win and self._lfm_win.winfo_exists():
            self._lfm_win.lift(); return
        win = tk.Toplevel(self.root); win.title("OTERNOS // LAST.FM AUTH")
        win.configure(bg=C["bg"]); win.geometry("420x200"); win.resizable(False, False)
        self._lfm_win = win
        tk.Label(win, text="LAST.FM", font=FML, fg=C["white"], bg=C["bg"]).pack(pady=14)
        tk.Label(win, text="Configure Last.fm API credentials in Settings\nto enable scrobbling.",
                 font=FM, fg=C["white2"], bg=C["bg"]).pack(pady=10)
        tk.Button(win, text="OPEN SETTINGS", font=FM, fg=C["white"], bg=C["border"],
                  relief="flat", bd=0, command=lambda: (win.destroy(), self._open_settings()),
    ...
    def _on_rclick(self, event):
        ...
        t = self.library[lib_idx]
        m = tk.Menu(
            self.root, tearoff=0, bg=C["panel"], fg=C["white"], font=FM, bd=0,
            relief="flat", activebackground=C["select"], activeforeground=C["white"],
        )
        m.add_command(label="▶  Play Now", command=lambda: (
            self.queue.__setitem__(slice(None), list(self._display_indices)),
            self.queue_pos.__class__,
            self._play_item(self._display_indices.index(lib_idx) if lib_idx in self._display_indices else 0, force=True)
        ))
        m.add_command(label="＋  Add to Queue", command=lambda: (
            self.queue.append(lib_idx), self._refresh_queue() if hasattr(self, "_refresh_queue") else None
        ))
        m.add_separator()
        m.add_command(label="＋  Add to Playlist", command=lambda: self._add_to_pl_dialog(lib_idx))
        m.add_separator()
        m.add_command(label="✎  Edit Tags", command=lambda: self._open_tag_editor(lib_idx) if hasattr(self, "_open_tag_editor") else None)
        m.add_command(label="✕  Remove from Library", command=lambda: self.lib_ctrl.remove_track(lib_idx))
        try:
    ...
    def _ab_set_a(self):
        pos = self.engine.get_position() if not self._sp_mode else self._sp_pos_ms / 1000
        self._ab_a = pos
    ...
    def _add_to_queue(self, lib_idx):
        if lib_idx not in self._manual_queue:
    ...
    def _alb_open_album(self, album_name, idxs):
        ...
        for txt, wd in [("#", 3), ("TITLE", 30), ("ARTIST", 20), ("TIME", 6)]:
            tk.Label(hdr, text=txt, font=("Courier New", 8, "bold"),
                     fg=C["white3"], bg=C["bg"], width=wd, anchor="w").pack(side="left")
        ...
        for pos, idx in enumerate(idxs):
            ...
            row = tk.Frame(self._alb_inner,
                           bg=C["panel"] if pos % 2 == 0 else C["panel2"],
                           cursor="hand2")
            row.pack(fill="x", padx=16, pady=1)
            tk.Label(row, text=f"{pos+1:>3}", font=FM, fg=C["white3"],
                     bg=row["bg"], width=3, anchor="w").pack(side="left", padx=(6, 0))
            tk.Label(row, text=title[:36], font=FM, fg=C["white"],
                     bg=row["bg"], width=30, anchor="w").pack(side="left", padx=4)
            tk.Label(row, text=artist[:22], font=FM, fg=C["white2"],
                     bg=row["bg"], width=20, anchor="w").pack(side="left")
            tk.Label(row, text=dur_str, font=FM, fg=C["white3"],
                     bg=row["bg"], width=6, anchor="w").pack(side="left")

            def _play(e, i=idx, il=idxs, p=pos):
                self.queue = list(il); self.queue_pos = p; self._play_item(i)

            row.bind("<Double-Button-1>", _play)
            row.bind("<Enter>",  lambda e, r=row: r.config(bg=C["select"]))
            row.bind("<Leave>",  lambda e, r=row, p=pos:
                     r.config(bg=C["panel"] if p % 2 == 0 else C["panel2"]))
        ...
        def _play_all():
            self.queue = list(idxs); self.queue_pos = 0; self._play_item(idxs[0])

        def _shuffle_all():
            import random
            s = list(idxs); random.shuffle(s)
            self.queue = s; self.queue_pos = 0; self._play_item(s[0])

        for txt, cmd in [("▶  PLAY ALL", _play_all), ("⇄  SHUFFLE", _shuffle_all)]:
            b = tk.Label(btn_row, text=txt, font=FM, fg=C["white"],
                         bg=C["panel2"], padx=12, pady=4, cursor="hand2")
    ...
    def _alb_rename(self, old_name, idxs):
        new_name = tk.simpledialog.askstring(
            "Rename Album", f"New name for '{old_name}':",
    ...
    def _alb_set_art_file(self, album_name):
        ...
        path = filedialog.askopenfilename(
            ...
            parent=self.root)
        if not path: return
        if not hasattr(self, "_album_meta"): self._album_meta = {}
        if album_name not in self._album_meta: self._album_meta[album_name] = {}
    ...
    def _alb_show_context(self, event, album_name, idxs):
        ...
        m.add_command(label="▶  Play Album",
                      command=lambda: [setattr(self, "queue", list(idxs)),
                                       setattr(self, "queue_pos", 0),
                                       self._play_item(idxs[0])])
        m.add_separator()
        m.add_command(label="♥  Remove Favorite" if is_fav else "♡  Add to Favorites",
                      command=lambda: self._alb_toggle_fav(album_name))
        m.add_separator()
        m.add_command(label="✎  Rename Album",
                      command=lambda: self._alb_rename(album_name, idxs))
        m.add_command(label="🖼  Set Art from File",
                      command=lambda: self._alb_set_art_file(album_name))
        m.add_command(label="🔗  Set Art from URL",
                      command=lambda: self._alb_set_art_url(album_name))
        m.add_command(label="✕  Clear Custom Art",
                      command=lambda: self._alb_clear_art(album_name))
        m.add_separator()
        m.add_command(label=f"  {len(idxs)} tracks · {album_name[:28]}", state="disabled")
        try:
            m.tk_popup(event.x_root, event.y_root)
        finally:
            m.grab_release()
    ...
    def _alb_show_grid(self):
        ...
        if not albums:
            tk.Label(self._alb_inner, text="No albums in library.",
                     font=FM, fg=C["white3"], bg=C["bg"]).pack(pady=40)
            return
        ...
        for col_idx, (album_name, idxs) in enumerate(
            ...
            art_cv = tk.Canvas(cell, bg=C["panel"], width=CELL_W, height=104,
            ...
            heart = tk.Label(fav_row, text="♥" if is_fav else "♡",
                             font=("Courier New", 9),
            ...
            lbl = tk.Label(cell, text=album_name[:22], font=FMS,
                           ...
                           wraplength=CELL_W - 8, justify="left", anchor="w")
    ...
    def _alb_toggle_fav(self, album_name):
        if not hasattr(self, "_album_meta"): self._album_meta = {}
        if album_name not in self._album_meta: self._album_meta[album_name] = {}
        self._album_meta[album_name]["fav"] = not self._album_meta[album_name].get("fav", False)
        self._save()
        self._alb_show_grid()
    ...
    def _apply_nier_theme(self):
        ...
        _nf = next((f for f in ("Consolas","Lucida Console","Courier New") if self._font_exists(f)), "Courier New")
        FM  = (_nf, 9);  FMS = (_nf, 8)
        FML = (_nf, 10, "bold");  FMX = (_nf, 13, "bold")

        GOLD  = "#c8b878";  DGOLD = "#504838";  CREAM = "#e8e4d0"
        ...
        try:
            for w in self.root.winfo_children():
                if w.winfo_class() == "Frame":
                    for child in w.winfo_children():
                        if isinstance(child, tk.Label) and "OTERNOS" in str(child.cget("text")):
                            child.config(text="Y O R H A",
                                         font=(_nf, 9, "bold"), fg=GOLD)
                            break
                    break
        except Exception:
            pass
        ...
        try:
            for key, btn in self.tab_btns.items():
                is_active = (key == getattr(self, "view", ""))
                btn.config(font=(_nf, 7, "bold"),
                           fg=GOLD if is_active else DGOLD)
                btn.unbind("<Enter>"); btn.unbind("<Leave>")
        ...
        try:
            for btn in (self.btn_shuf, self.btn_prev, self.btn_play, self.btn_next, self.btn_repeat):
                ...
                btn.unbind("<Enter>"); btn.unbind("<Leave>")
                btn.bind("<Enter>", lambda e, w=btn: ColorAnim.run(self.root, w, "fg", DGOLD, GOLD, duration_ms=60))
                btn.bind("<Leave>", lambda e, w=btn: ColorAnim.run(self.root, w, "fg", GOLD, DGOLD, duration_ms=60))
        except Exception:
            pass
    ...
    def _apply_playback_speed(self):
        ...
        self.engine.set_speed(
            ...
            on_done=lambda: self.root.after(0, lambda: self._set_status(
                f"SPEED {self._playback_speed}×" if self._playback_speed != 1.0 else "SPEED NORMAL")),
            on_error=lambda msg: self.root.after(0, lambda: self._set_status("SPEED ERROR"))
        )
    ...
    def _build_album_view(self):
        ...
        self._alb_hdr_lbl = tk.Label(hdr, text="ALBUMS", font=FMX,
                                      fg=C["white"], bg=C["bg"])
        self._alb_hdr_lbl.pack(side="left")
        self._alb_back_btn = tk.Label(hdr, text="← BACK", font=FM,
                                       fg=C["white3"], bg=C["bg"], cursor="hand2")
        self._alb_back_btn.pack(side="left", padx=(16, 0))
        self._alb_back_btn.pack_forget()
        self._alb_back_btn.bind("<Button-1>", lambda e: self._alb_show_grid())
        self._alb_back_btn.bind("<Enter>", lambda e: self._alb_back_btn.config(fg=C["white"]))
        self._alb_back_btn.bind("<Leave>", lambda e: self._alb_back_btn.config(fg=C["white3"]))
        ...
        self._alb_ctx = tk.Menu(self.root, tearoff=0,
    ...
    def _cycle_speed(self):
        ...
        if not self.engine._path:
            self._set_status(f"SPEED QUEUED: {self._playback_speed}×" if self._playback_speed != 1.0 else "SPEED: NORMAL")
            return
    ...
    def _draw_album_art_async(self, cv, idxs, w, h, album_name=""):
        ...
        def _draw_placeholder():
            ...
            rng  = _r.Random(seed)
            for _ in range(8):
                x1 = rng.randint(-10, w); y1 = rng.randint(-10, h)
                x2 = x1 + rng.randint(20, 80); y2 = y1 + rng.randint(20, 60)
            ...
            for _ in range(4):
                ...
                r = rng.randint(12, 35); br = rng.randint(25, 55)
                cv.create_oval(x-r, y-r, x+r, y+r,
                               fill=f"#{br:02x}{br:02x}{br:02x}", outline="")
            initial = (album_name[:1] or "?").upper()
            cv.create_text(w//2, h//2, text=initial,
                           font=("Courier New", 36, "bold"), fill=C["white3"])
    ...
    def _draw_prog(self, r):
        ...
        self.prog_cv.coords(self.prog_fill, 0, 0, x, 6)
        self.prog_cv.coords(self.prog_dot,  x-5, -2, x+5, 8)
    ...
    def _next(self):
        ...
        if self._active_source in ("youtube", "soundcloud"):
            ...
            return
        if not self.queue and not self._manual_queue:
            return
    ...
    def _poll(self):
        if self._sp_mode:
            ...
            if self.engine.is_done() and not getattr(self, "_speed_processing", False):
    ...
    def _post_theme_refresh(self):
        ...
        try:
            ...
            if hasattr(self, "prog_dot"):
                self.prog_cv.itemconfig(self.prog_dot, fill=C["white"], outline=C["glow"])
            # Redraw vol canvas
    ...
    def _q_drag_motion(self, event): pass

    def _q_drag_press(self, event): pass

    def _q_drag_release(self, event): pass
                    ...
                    if cur_fg in _all_theme_colors:
                        ...
                        for preset in self._theme_presets.values():
                            if cur_fg == preset.get("white","").lower():
                                new_fg = C["white"]; break
                            elif cur_fg == preset.get("white2","").lower():
                                new_fg = C["white2"]; break
                            elif cur_fg == preset.get("white3","").lower():
                                new_fg = C["white3"]; break
                            elif cur_fg == preset.get("glow","").lower():
                                new_fg = C["white2"]; break  # reset glow → neutral after theme swap
                    cfg={}
    ...
    def _restore_cybercore(self):
        ...
        try:
            for w in self._side_frame.winfo_children():
                for child in (w.winfo_children() if hasattr(w,"winfo_children") else []):
                    if isinstance(child,tk.Label):
                        t=child.cget("text")
                        if t in ("初音ミク","NERV","YoRHa"): child.config(text="OTERNOS",font=("Courier New",8,"bold"),fg=C["white"])
                        elif any(x in str(t) for x in ("VOCALOID","God's In His Heaven","MAGI v","No. 2")): child.config(text="SYS v1.0",font=("Courier New",6),fg=C["white3"])
        except Exception:
            pass
                ...
                if w.winfo_class() == "Frame":
                    for child in w.winfo_children():
                        if isinstance(child, tk.Label):
                            ...
                            if "M I K U" in t or "N E R V" in t or "Y O R H A" in t:
        ...
        try:
            ...
            if getattr(self,"_viz_mode","").startswith(("miku","nerv_","nier_")):
        ...
        try:
            ...
            elif cur and cur.startswith("// "): self.now_title.config(text=cur[3:])
        except Exception:
            pass
    ...
    def _set_status(self, text):
        ...
        ColorAnim.run(self.root, self.status_lbl, "fg", C["white"], C["white2"], duration_ms=600)
    ...
    def _upd_vol(self):
        ...
        cv.create_text(off, cy - 15, text="VOL",
                       anchor="w", font=("Courier New", 9, "bold"), fill=lc)
        cv.create_text(off + W, cy - 15, text=f"{int(vol*100):>3}%",
                       anchor="e", font=("Courier New", 9, "bold"), fill=lc)
    ...
    def _viz_nier_machine(self, cv, W, H, t, playing, bars):
        ...
        for i, (x1, y1) in enumerate(nodes):
            for j, (x2, y2) in enumerate(nodes):
                ...
                cv.create_line(x1, y1, x2, y2,
                               fill=f"#{ev:02x}{int(ev*0.88):02x}{int(ev*0.5):02x}", width=1)
        ...
        for i, (nx, ny) in enumerate(nodes):
            ...
            cv.create_oval(nx-nr, ny-nr, nx+nr, ny+nr,
                           fill=f"#{nv:02x}{int(nv*0.88):02x}{int(nv*0.5):02x}", outline="")
        ...
        if len(nodes) > 1:
            ...
            cv.create_oval(px-pr, py-pr, px+pr, py+pr,
                           fill=f"#{pv:02x}{int(pv*0.9):02x}{int(pv*0.55):02x}", outline="")

        dv = int(55 + energy * 70)
        dc = f"#{dv:02x}{int(dv*0.85):02x}{int(dv*0.45):02x}"
        labels = ["CONNECTED","TRANSMITTING","SEARCHING","NETWORK OK"]
        cv.create_text(W//2, H-12, text=f"// MACHINE NETWORK — {labels[(t//150)%4]} //",
                       font=("Consolas", max(6, int(W*0.013))), fill=dc, anchor="center")

    def _viz_nier_ruins(self, cv, W, H, t, playing, bars):
        import math as _m, random as _random
        bass   = sum(bars[:8])  / 8  if bars else 0
        energy = sum(bars)      / len(bars) if bars else 0
        ...
        if bars:
            ...
            if len(pts) >= 4:
                ...
                cv.create_line(pts, fill=f"#{wv:02x}{int(wv*0.9):02x}{int(wv*0.55):02x}",
                               width=2, smooth=True)
            # Mirror
            pts2 = []
            for i, h in enumerate(bars):
            ...
            if len(pts2) >= 4:
                ...
                cv.create_line(pts2, fill=f"#{wv2:02x}{int(wv2*0.9):02x}{int(wv2*0.55):02x}",
                               width=1, smooth=True)

        # Status line
        dv = int(60 + energy * 70)
        dc = f"#{dv:02x}{int(dv*0.85):02x}{int(dv*0.45):02x}"
        msgs = ["SYSTEM FAILURE","OBJECTIVE UNKNOWN","SEARCHING FOR PURPOSE","DATA CORRUPTED","..."]
        cv.create_text(W//2, H-12,
                       text=f"[ {msgs[(t//180)%len(msgs)]} ]",
                       font=("Consolas", max(6, int(W*0.013))),
                       fill=dc, anchor="center")

    def _viz_nier_yorha(self, cv, W, H, t, playing, bars):
        import math as _m
        bass   = sum(bars[:8])  / 8  if bars else 0
        energy = sum(bars)      / len(bars) if bars else 0
        GOLD   = "#c8b878";  DGOLD = "#3a3020";  CREAM = "#e8e4d0"
        ...
        cv.create_polygon(pts, outline=f"#{gv:02x}{int(gv*0.9):02x}{int(gv*0.55):02x}", fill="", width=1)
        ...
        cv.create_polygon(pts2, outline=f"#{mv:02x}{int(mv*0.9):02x}{int(mv*0.5):02x}", fill="", width=1)
        ...
        cv.create_oval(cx-cr, cy-cr, cx+cr, cy+cr,
                       fill=f"#{cv2:02x}{int(cv2*0.9):02x}{int(cv2*0.55):02x}", outline="")
        ...
        if n:
            ...
            for i, h in enumerate(bars):
                ...
                bx = 40 + i * bw
                fv = int(60 + h * 140)
                cv.create_rectangle(bx, H - 20 - bh, bx + bw - 1, H - 20,
                                    fill=f"#{fv:02x}{int(fv*0.88):02x}{int(fv*0.5):02x}", outline="")

        # Corner data
        dv = int(60 + energy * 80)
        dc = f"#{dv:02x}{int(dv*0.85):02x}{int(dv*0.45):02x}"
        unit_text = ["No. 2 Type B", "No. 9 Type S", "YoRHa Unit", "Bunker Comm"]
        cv.create_text(8, 8, text=f"[ {unit_text[(t//200)%4]} ]",
                       font=("Consolas", max(6, int(W*0.013))), fill=dc, anchor="nw")
        cv.create_text(W-8, 8, text=f"// {int(energy*100):03d} //",
                       font=("Consolas", max(6, int(W*0.013))), fill=dc, anchor="ne")
        cv.create_text(W-8, H-8, text="Glory to Mankind",
                       font=("Consolas", max(6, int(W*0.012))), fill=dc, anchor="se")
    ...
    def _on_close(self):
        ...
        # Cancel all pending Tk after-jobs before destroy
        for attr in (
            "_scrobble_job",
            "_sleep_job",
            "_sp_np_job",
    ...
    # ── PERSISTENCE ───────────────────────
    def _load_data(self):
        """Delegated to LibraryController. Kept for call-site compatibility."""
        self.ui_sounds = UISounds(volume=0.18)  # temp default; updated after load
        self.lib_ctrl.load()
        # Refresh ui_sounds with loaded settings
        ...
        def _run_next(steps):
            ...
            try:
                ...
                try:
                    ...
                    pass
            self.root.after(30, lambda: _run_next(steps[1:]))
    ...
    def _build_lib_view(self):
        ...
        sys_row.pack(fill="x")
        tk.Label(
            sys_row, text="VIEW::", font=("Courier New", 8), fg=C["white3"], bg=C["bg"]
        ).pack(side="left")
    ...
    def _build_queue_view(self):
        ...
        cl = tk.Label(
            hdr, text="clear all", font=FMS, fg=C["white3"], bg=C["bg"], cursor="hand2"
        )
        cl.pack(side="right")
    ...
    def _build_visualizer_view(self):
        ...
        self.viz_mode_lbl = tk.Label(
            hdr, text="[ HEXCORE ]", font=FMS, fg=C["white3"], bg=C["bg"]
        )
    ...
    def _build_soundcloud_view(self):
        ...
        self.sc_status_lbl = tk.Label(top, text="[ PUBLIC ACCESS ]", font=FMS, fg=C["white3"], bg=C["bg"])
        self.sc_status_lbl.pack(side="left", padx=12)
        self.sc_queue_lbl = tk.Label(top, text="", font=FMS, fg=C["white3"], bg=C["bg"])
        self.sc_queue_lbl.pack(side="left")

        # ── Collapsible auth row (hidden by default) ──────────────────────
        self._sc_auth_visible = False
        self.sc_auth_row = tk.Frame(self.sc_frame, bg=C["bg"])
        tk.Label(self.sc_auth_row, text="OAuth Token:", font=FMS, fg=C["white3"], bg=C["bg"]).pack(side="left", padx=(20, 4))
        self.sc_token_var = tk.StringVar(value=self.sc_api.oauth_token or "")
        sc_tok_entry = tk.Entry(self.sc_auth_row, textvariable=self.sc_token_var,
                                font=FMS, bg=C["panel"], fg=C["white"],
                                insertbackground=C["white"], relief="flat", bd=0, width=36, show="*")
        sc_tok_entry.pack(side="left", ipady=3, padx=(0, 6))
        sc_save = tk.Label(self.sc_auth_row, text="[SAVE]", font=FMS, fg=C["white3"], bg=C["bg"], cursor="hand2")
        ...
        tk.Label(self.sc_auth_row, text="  from sc-inspector or browser devtools",
        ...
        sc_go = tk.Label(self.sc_search_frame, text="[GO]", font=FMS, fg=C["white3"], bg=C["bg"], cursor="hand2", padx=8)
    ...
    def _sc_do_search(self):
        ...
        def _fetch():
            ...
            self.root.after(0, lambda: self._sc_show_tracks(tracks, f'RESULTS FOR "{q.upper()}"'))
    ...
    def _sc_load_likes(self):
        if not self.sc_api.oauth_token:
            self.sc_list.delete(0, "end")
            self.sc_list.insert("end", "  Enter your OAuth token ([ TOKEN ] button) to load likes.")
            return
    ...
    def _sc_show_tracks(self, tracks, label=""):
        ...
        for i, t in enumerate(tracks):
            ...
            self.sc_list.insert(
                "end",
                f" {heart}{queued} {i+1:>3}.  {title:<36}  {artist:<22}  {dur_str}  ▸{plays_str}{genre_part}"
            )
    ...
    def _stop_all_sources(self):
        """Delegated to SourceController."""
        self.source_ctrl.stop_all()
    ...
    def _sc_stream(self, track):
        ...
        try:
            ...
            def _reporthook(block_num, block_size, total):
                ...
                if total > 0:
                    pct = min(100, int(downloaded[0] * 100 / total))
                    self.root.after(0, lambda p=pct: self.sc_np_lbl.config(text=f"  Buffering... {p}%"))
                else:
                    mb = downloaded[0] / 1024 / 1024
                    self.root.after(0, lambda m=mb: self.sc_np_lbl.config(text=f"  Buffering... {m:.1f}MB"))

            tmp = tempfile.NamedTemporaryFile(suffix=".mp3", delete=False)
            tmp.close()
            _ur.urlretrieve(stream_url, tmp.name, reporthook=_reporthook)
            ...
            def _play():
                ...
                if self.engine.load(tmp.name):
                    ...
                    self._draw_prog(0)
                    self.sc_np_lbl.config(text=f"▶  {title[:40]}  —  {artist[:24]}")
            ...
            self.root.after(0, lambda: (
                self.sc_np_lbl.config(text=f"  Stream error: {ex}"),
    ...
    def _build_youtube_view(self):
        ...
        go = tk.Label(
            sf, text="GO", font=FMS, fg=C["white3"], bg=C["bg"], cursor="hand2", padx=10
        )
        go.pack(side="left")
    ...
    def _build_stream_view(self):
        ...
        self._stream_sc_btn  = tk.Label(sw, text="[ SOUNDCLOUD ]", font=FM, fg=C["white3"], bg=C["bg"], cursor="hand2", padx=4)
        self._stream_dz_btn  = tk.Label(sw, text="[ DEEZER ]",     font=FM, fg=C["white3"], bg=C["bg"], cursor="hand2", padx=4)
        self._stream_ia_btn  = tk.Label(sw, text="[ ARCHIVE ]",    font=FM, fg=C["white3"], bg=C["bg"], cursor="hand2", padx=4)
        self._stream_bc_btn  = tk.Label(sw, text="[ BANDCAMP ]",   font=FM, fg=C["white3"], bg=C["bg"], cursor="hand2", padx=4)
    ...
    def _build_deezer_view(self):
        ...
        self.dz_status_lbl = tk.Label(top, text="[ 30s PREVIEWS ]", font=FMS, fg=C["white3"], bg=C["bg"])
        ...
        dz_go = tk.Label(self.dz_search_frame, text="[GO]", font=FMS, fg=C["white3"], bg=C["bg"], cursor="hand2", padx=8)
        ...
        self.dz_list.bind("<MouseWheel>", lambda e: self.dz_list.yview_scroll(int(-1*(e.delta/120))*3, "units"))
    ...
    def _build_archive_view(self):
        ...
        self.ia_status_lbl = tk.Label(top, text="[ FREE / OPEN ]", font=FMS, fg=C["white3"], bg=C["bg"])
        ...
        ia_go = tk.Label(sf, text="[GO]", font=FMS, fg=C["white3"], bg=C["bg"], cursor="hand2", padx=8)
        ...
        self.ia_items_list.bind("<MouseWheel>", lambda e: self.ia_items_list.yview_scroll(int(-1*(e.delta/120))*3, "units"))
        ...
        self.ia_files_list.bind("<MouseWheel>", lambda e: self.ia_files_list.yview_scroll(int(-1*(e.delta/120))*3, "units"))
    ...
    def _build_bandcamp_view(self):
        ...
        self.bc_list.bind("<MouseWheel>", lambda e: self.bc_list.yview_scroll(int(-1*(e.delta/120))*3, "units"))
    ...
    def _build_claude_view(self):
        ...
        self._ctx_pos = tk.Label(
            ctx_left, text="", font=("Courier New", 7), fg=C["white3"], bg=C["panel"]
        )
    ...
    def _build_playerbar(self):
        ...
        right = tk.Frame(inner, bg=C["panel"], width=220)
    ...
    def _open_lyric_popup(self):
        ...
        self._lp_badge = tk.Label(
            hdr, text="", font=("Courier New", 7), fg=C["white3"], bg=C["panel"]
        )
        ...
        self._lp_sync_tick()
        pop.protocol(
            "WM_DELETE_WINDOW", lambda: (self._lp_cancel_sync(), pop.destroy())
        )
    ...
    def _set_source_badge(self, source):
        """Delegated to SourceController._update_badge (kept for call-site compat)."""
        self.source_ctrl._update_badge()
    ...
    def _refresh_queue(self):
        ...
        if self._manual_queue:
            ...
            tk.Label(
                self.queue_inner,
                text="  Right-click any track → Add to Queue",
                font=FMS,
                fg=C["white3"],
                bg=C["bg"],
        ...
        if self.queue:
            ...
            tk.Label(
                self.queue_inner,
                text="  Play any track to populate",
                font=FMS,
                fg=C["white3"],
                bg=C["bg"],
            ).pack(anchor="w", padx=8, pady=4)
    ...
    def _create_playlist(self):
        ...
        tk.Label(
            dlg, text="PLAYLIST NAME", font=FM, fg=C["white3"], bg=C["panel"]
        ).pack(pady=(18, 6))
        var = tk.StringVar()
        e = tk.Entry(
            dlg,
    ...
    def _add_to_pl_dialog(self, lib_idx):
        ...
        tk.Label(
            dlg, text="SELECT PLAYLIST", font=FM, fg=C["white3"], bg=C["panel"]
        ).pack(pady=(12, 6))
        lb = tk.Listbox(
            dlg,
            bg=C["bg"],
    ...
    def _add_files(self):
        ...
        if paths:
            self.lib_ctrl.import_paths(list(paths))

    def _add_folder(self):
        folder = filedialog.askdirectory(title="Select Music Folder")
        if folder:
            self.lib_ctrl.import_folder(folder)

    def _import(self, paths):
        """Delegated to LibraryController."""
        self.lib_ctrl.import_paths(paths)

    def _import_batch(self, paths, progress=False):
        """Delegated to LibraryController."""
        self.lib_ctrl._import_batch(paths, progress)
    ...
    def _play_item(self, pos, force=False):
        """Delegated to PlaybackController."""
        self.playback_ctrl.play_item(pos, force)

    def _do_load_track(self, track, user_initiated=False):
        """Delegated to PlaybackController."""
        self.playback_ctrl.do_load_track(track, user_initiated)
