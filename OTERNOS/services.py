...
services.py
─────────────────────────────────────────────────────────────────────────────
OTERNOS PLAYER  —  Background service integrations.
  • GlobalHotkeys    — Windows system-wide media key registration
  • LastFmScrobbler  — Last.fm auth + scrobbling
  • DiscordRPC       — Discord rich presence via IPC pipe
  • FolderWatcher    — polling-based audio file watcher
─────────────────────────────────────────────────────────────────────────────
"""
...
# ─────────────────────────────────────────
class GlobalHotkeys:
    """Register system-wide media key hotkeys via RegisterHotKey (Windows)."""
    MOD_NOREPEAT        = 0x4000
    ...
    def __init__(self, callbacks):
        """callbacks: {id: callable}  ids 1-4 → play/next/prev/stop"""
    ...
    def _run(self):
        try:
            ...
            registered = [hid for hid, mod, vk in keys
            ...
            pass   # silently no-op on non-Windows
...
class LastFmScrobbler:
    """Handles Last.fm auth (md5-based) + scrobbling + now-playing updates."""
    ...
    def __init__(self, api_key="", api_secret="", session_key=""):
        ...
        self._current    = None   # (artist, title, album, dur, start_ts)
    ...
    def _call(self, params, post=False):
        ...
        try:
            req = (urllib.request.Request(self.ROOT, data=data) if post
                   else urllib.request.Request(self.ROOT + "?" + urllib.parse.urlencode(params)))
    ...
    def now_playing(self, artist, title, album="", duration=0):
        ...
        params = {"method": "track.updateNowPlaying", "artist": artist,
                  "track": title, "sk": self.session_key}
        if album:    params["album"]    = album
        if duration: params["duration"] = str(int(duration))
        threading.Thread(target=lambda: self._call(params, post=True), daemon=True).start()

    def scrobble(self, artist, title, album="", duration=0, timestamp=None):
        if not self.ready: return
        ts = str(int(timestamp or time.time()))
        params = {"method": "track.scrobble", "artist": artist,
                  "track": title, "timestamp": ts, "sk": self.session_key}
        if album:    params["album"]    = album
        if duration: params["duration"] = str(int(duration))
        threading.Thread(target=lambda: self._call(params, post=True), daemon=True).start()
    ...
    def track_started(self, track):
        ...
        self.now_playing(track.get("artist",""), track.get("title",""),
                         track.get("album",""),  track.get("duration", 0))
...
# ─────────────────────────────────────────
class DiscordRPC:
    """Minimal Discord IPC Rich Presence — no external libs required."""
            ...
            payload = {
                ...
                "args": {
                    ...
                    "activity": {
                        ...
                        "assets":     {"large_image": "oternos_logo",
                                       "large_text":  "OTERNOS PLAYER"},
                        "timestamps": {"start": now_ms - int(elapsed_s * 1000)},
                        "type": 2,
                    }
                },
                "nonce": str(self._seq),
    ...
    def _recv(self):
        try:
            ...
            return json.loads(self._sock.read(length))
        except Exception:
            return None
...
# ─────────────────────────────────────────
class FolderWatcher:
    """Watches directories for new audio files and calls on_new(path)."""
