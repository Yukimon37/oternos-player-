...
class AudioEngine:
    ...
    def load(self, path):
        ...
        if getattr(self, "_preload_path", None) == path and getattr(
            ...
            try:
                ...
                print(f"[AudioEngine] load(preload) error: {e}")
                return False
        try:
            self.stop()
            with _silence_stderr():
    ...
    def pause(self):
        ...
        try:
            ...
            print(f"[AudioEngine] pause error: {e}")
    ...
    def unpause(self):
        ...
        try:
            ...
            print(f"[AudioEngine] unpause error: {e}")
    ...
    def _dbg(msg):
        try:
            ...
            with open(Path.home() / "voidplayer_seek_debug.log", "a") as f:
                f.write(line)
        except Exception:
            pass
    ...
    def seek(self, pos_s):
        ...
        self._dbg(f"[SEEK] requested={pos_s:.2f}s  was_paused={was_paused}  duration={self._duration:.2f}  path={self._path}")
        try:
            pygame.mixer.music.stop()
            if self._path:
            ...
            pygame.mixer.music.set_volume(self._volume)
            # Do NOT sleep here — sleeping on the Tk event thread freezes the UI
            # during scrubbing. Instead, anchor the clock to now and let get_pos()
            # catch up on the next poll tick (≤100 ms away).
            self._seek_offset = pos_s
            self._start_time = time.monotonic()
            raw_ms = 0  # get_pos() returns ~0 immediately after play(); use offset math instead
            ...
            self._dbg(f"[SEEK] done  _seek_offset={self._seek_offset:.2f}  _pos_cache={self._pos_cache:.2f}  is_playing={self._is_playing}  is_paused={self._is_paused}")
        except Exception as e:
            self._dbg(f"[SEEK] ERROR: {e}")
